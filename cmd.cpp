...
std::string print(std::string str, bool toIncludeEndl){
    ...
    return str;}
std::string winver(){print("Windows 11 22H2 (Build 22621.1265)\nCommand Prompt v0.1", true); return "winver()";}
std::string changeDir(std::string path){currentWorkingDirectory += path; return "changeDir()"; }
...
std::string echo() { print(">_", false); std::string line1{};  getline(std::cin, line1); print("specify output stream>", false); std::string output{}; std::cin >> output; if (output == "stdout" || output == "cout" || output == "std::cout") { print(line1, true); }
else { std::ofstream os(output); os << line1; os.close(); }
                    return "echo function!";}
int main(){
    const char RealUsername[9] {"FooUser1"};
    const char RealPassword[9] {"1234!@#$"};
    ...
    print("Please enter your password: ", false);
    ...
    else { print("The username or password or both was incorrect, rerun the program and enter the correct credentials! Press any key to restart!>", true); std::string i{}; std::cin >> i; return 0; }
